...
static void rna_def_space_view3d_overlay(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(prop, "Wire Opacity", "Bone wireframe opacity");
  RNA_def_property_range(prop, 0.0f, 2.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1.0f, 1, 2);
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_GPencil_update");

  prop = RNA_def_property(srna, "bone_wire_fade", PROP_FLOAT, PROP_FACTOR);
  RNA_def_property_float_sdna(prop, NULL, "overlay.bone_wire_fade");
  RNA_def_property_ui_text(
      prop, "Depth Fade", "Bone wireframe opacity fades to zero at the far clip plane");
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_GPencil_update");
